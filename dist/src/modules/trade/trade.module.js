"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TradeModule = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const trade_service_1 = require("./trade.service");
const trade_controller_1 = require("./trade.controller");
const trade_entity_1 = require("./entities/trade.entity");
const project_entity_1 = require("../project/entities/project.entity");
const transaction_service_1 = require("../transactions/transaction.service");
const transaction_entity_1 = require("../transactions/entities/transaction.entity");
let TradeModule = class TradeModule {
};
TradeModule = __decorate([
    (0, common_1.Module)({
        imports: [typeorm_1.TypeOrmModule.forFeature([project_entity_1.ProjectEntity, trade_entity_1.TradeEntity, transaction_entity_1.TransactionEntity])],
        controllers: [trade_controller_1.TradeController],
        providers: [trade_service_1.TradeService, transaction_service_1.TransactionService],
    })
], TradeModule);
exports.TradeModule = TradeModule;
//# sourceMappingURL=trade.module.js.map